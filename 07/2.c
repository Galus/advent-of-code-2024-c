...
char *calculate(unsigned long long goal, unsigned long long total, int *right,
  ...
  char catBuff[999];
  if (rightIdx == 0) {
    sprintf(catBuff, "%d", right[rightIdx]);
  } else {
    sprintf(catBuff, "%s || %d", exprec, right[rightIdx]);
  }
  char catNum[30];
  sprintf(catNum, "%llu%d", total, right[rightIdx]);
  ...
  /*printf("right: %d\n", right[rightIdx]);*/
  /*printf("cat: %llu\n", cat);*/
  catExp = calculate(goal, cat, right, rightCount, rightIdx + 1, catBuff);
